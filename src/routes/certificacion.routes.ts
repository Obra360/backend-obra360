...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const companyObraIds = companyObras.map((obra) => obra.id);

      const where: any = {
        obra: { in: companyObraIds },
      };

      if (obra && typeof obra === "string") where.obra = obra;
      if (estado && typeof estado === "string")
        where.estado = estado as EstadoCertificacion;
      if (fecha && typeof fecha === "string")
        where.fecha = new Date(fecha + "T00:00:00.000Z");

      if (mes && ano && typeof mes === "string" && typeof ano === "string") {
        const startDate = new Date(
          `${ano}-${mes.padStart(2, "0")}-01T00:00:00.000Z`,
        );
      ...
      const certificaciones = await prisma.certificacion.findMany({
        ...
        include: {
          ...
          user: {
            // CORREGIDO: la relación se llama 'user'
            select: {
              id: true,
              firstName: true,
              lastName: true,
      ...
      const certificacionesFormateadas = certificaciones.map((cert) => ({
        ...
        createdBy: cert.user
          ? {
              // CORREGIDO: acceder a 'cert.user'
              id: cert.user.id,
              nombre: `${cert.user.firstName} ${cert.user.lastName}`,
              email: cert.user.email,
            }
          : null,
        items: cert.items.map((item: ItemCertificado) => ({
          // CORREGIDO: Añadido tipo explícito
          id: item.id,
          codigo: item.codigo,
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      const { obra, tipoCertificacion, fecha, items }: CreateCertificacionData =
        req.body;
      const currentUser = req.user;
      ...
      if (
        ...
        res.status(400).json({
          error:
            "Faltan campos obligatorios: obra, tipoCertificacion, fecha, items",
        });
        return;
      }
      ...
      if (!certificacionCompleta) {
        res.status(404).json({
          error: "No se pudo encontrar la certificación recién creada.",
        });
        return;
      }
      ...
      res.status(201).json({
        ...
        certificacion: {
          ...
          createdBy: certificacionCompleta.user // CORREGIDO: acceder a 'user'
            ? `${certificacionCompleta.user.firstName} ${certificacionCompleta.user.lastName}`
            : null,
        },
      });
    } catch (error) {
...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const obraExists = await prisma.obra.findFirst({
        where: {
      ...
      if (
        ...
        certificacion.createdBy !== currentUser.id
      ) {
        res
          .status(403)
          .json({ error: "Solo puedes ver certificaciones que has creado" });
        return;
      }
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const certificacionExistente = await prisma.certificacion.findUnique({
        where: { id },
      });
      ...
      if (
        currentUser &&
        currentUser.role === "OPERARIO" &&
        certificacionExistente.createdBy !== currentUser.id
      ) {
        res
          .status(403)
          .json({ error: "Solo puedes editar certificaciones que has creado" });
        return;
      }

      const datosActualizacion: any = {};
      if (updateData.obra) datosActualizacion.obra = updateData.obra;
      if (updateData.tipoCertificacion)
        datosActualizacion.tipoCertificacion = updateData.tipoCertificacion;
      if (updateData.fecha)
        datosActualizacion.fecha = new Date(
          updateData.fecha + "T00:00:00.000Z",
        );
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (
        ...
        res.status(403).json({
          error: "Solo puedes eliminar certificaciones que has creado",
        });
        return;
      }
      ...
      if (["APROBADA", "PAGADA"].includes(certificacion.estado)) {
        res.status(400).json({
          error: "No se puede eliminar una certificación aprobada o pagada",
        });
        return;
      }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const [
        ...
        prisma.certificacion.count({
          where: { ...whereClause, estado: "COMPLETA" },
        }),
        prisma.certificacion.count({
